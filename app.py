...
def download_ffmpeg() -> None:
    ...
    open(os.getcwd() + '\\ffmpeg.zip', 'wb').write(r.content)
    print('downloaded, extracting...')

    # extract / unzip
    with zipfile.ZipFile(os.getcwd() + '\\ffmpeg.zip', 'r') as zip_ref:
        zip_ref.extractall(os.getcwd() + '\\ffmpeg')
    print('extracted, copying necessary files...')

    # copy necessary file
    for folder in os.listdir(os.getcwd() + '\\ffmpeg'):
        shutil.copyfile(
            os.getcwd() + f'\\ffmpeg\\{folder}\\bin\\ffmpeg.exe', os.getcwd() + '\\ffmpeg.exe')
        break
    ...
    try:
        ...
        os.unlink(os.getcwd() + '\\ffmpeg.zip')
...
def listen(filename):
    ...
    """
    return send_file(os.getcwd() + f"\\outputs\\{filename}", mimetype="audio/mp3", as_attachment=False)
